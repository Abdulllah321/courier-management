<!-- Topbar HTML -->
<header
    class="bg-white shadow-lg fixed top-0 left-64 right-0 flex justify-between items-center p-4 border-b border-gray-200 z-50">
    <div class="text-gray-800 font-semibold flex items-center">
        <i class="fas fa-user-circle text-xl mr-2"></i>
        <span id="username" class="text-lg">
            <?php
            if (!empty($_SESSION['role']) && $_SESSION['role'] == "agent") {
                echo htmlspecialchars($_SESSION['agent_username']);
            } else {
    ...
    <div class="flex items-center space-x-4">
        ...
        <button id="notificationsBtn" class="relative p-2 text-gray-600 hover:text-gray-800 transition duration-300">
            ...
            <span id="notificationBadge"
                class="absolute top-0 right-0 text-xs text-white w-5 h-5 flex items-center justify-center bg-red-600 rounded-full hidden"></span>
        </button>
...
<div id="logoutModal" class="fixed inset-0 bg-gray-900 bg-opacity-60 items-center justify-center hidden z-50">
    <div
        class="bg-white rounded-lg shadow-lg w-full max-w-sm p-6 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
        ...
        <div class="flex justify-end space-x-4">
            <button id="confirmLogout"
                class="bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 transition duration-300">Logout</button>
            <button id="cancelLogout"
                class="bg-gray-300 text-gray-800 px-4 py-2 rounded-lg hover:bg-gray-400 transition duration-300">Cancel</button>
        </div>
    </div>
</div>