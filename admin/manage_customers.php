...
$totalQuery = "SELECT COUNT(*) as total FROM customers WHERE deleted = false";
...
$query = "SELECT * FROM customers WHERE deleted = false LIMIT :start, :limit";
...
<body class="bg-gray-100">
    ...
    <main class="ml-64 p-6 mt-16">
            ...
            <table class="w-full text-left border-collapse ">
                ...
                <tbody id="customersTableBody">
                    <?php foreach ($customers as $customer): ?>
                        <tr class="hover:bg-gray-100 customer-row">
                            <td class="py-3 px-4 border-b whitespace-nowrap">
                                <?php echo htmlspecialchars($customer['first_name']); ?></td>
                            <td class="py-3 px-4 border-b whitespace-nowrap">
                                <?php echo htmlspecialchars($customer['last_name']); ?></td>
                            <td class="py-3 px-4 border-b whitespace-nowrap">
                                <?php echo htmlspecialchars($customer['email']); ?></td>
                            <td class="py-3 px-4 border-b whitespace-nowrap">
                                <?php echo htmlspecialchars($customer['phone']); ?></td>
                            <td class="py-3 px-4 border-b flex gap-2 flex-nowrap items-center">
                                <a href="view_customer.php?id=<?php echo $customer['customer_id']; ?>"
                                    class="flex gap-1 flex-nowrap text-blue-600 hover:underline"><i class="fas fa-eye"></i>
                                    View</a>
                                <a href="edit_customer.php?id=<?php echo $customer['customer_id']; ?>"
                                    class="flex gap-1 flex-nowrap text-blue-600 hover:underline ml-4"><i
                                        class="fas fa-edit"></i> Edit</a>
                                <a href="#" data-id="<?php echo $customer['customer_id']; ?>"
                                    class="flex gap-1 flex-nowrap text-red-600 hover:underline ml-4 delete-customer"><i
        ...
        <div class="flex justify-between items-center mt-4">
            ...
            <div>
                <span>Showing <?php echo $start + 1; ?> to <?php echo min($start + $limit, $totalRecords); ?> of
                    <?php echo $totalRecords; ?> items</span>
            </div>
            <div class="flex gap-4">
                <a href="?page=<?php echo min($totalPages, $page + 1); ?>" class="text-blue-600 hover:underline">Next
                    &gt;</a>
    ...
    <div id="confirmationModal" class="fixed inset-0 items-center justify-center bg-gray-600 bg-opacity-50 hidden">
        <div
            class="bg-white p-6 rounded-lg shadow-lg w-1/3 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
            <h2 class="text-xl font-semibold mb-4">Confirm Deletion</h2>
            <p id="confirmationMessage" class="mb-4">Are you sure you want to delete this customer?</p>
